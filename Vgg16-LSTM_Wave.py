...
# Streamlit App
st.image("cancer.jpg", width=700)  # Change "cancer.jpg" to the path of your logo image file
...
# Store caption in session_state
    ...
    if st.button("Generate Caption"):
        with st.spinner("Generating caption..."):
            try:
                ...
                st.write(f"**Generated Caption:** {st.session_state.caption}")
            except Exception as e:
                st.error(f"Error generating caption: {e}")

    # Display the generated caption (always visible after generation)
    ...
    if st.session_state.caption:
        if st.button("Generate Audio"):
            with st.spinner("Converting caption to audio..."):
                try:
                    # Generate WAV audio from the caption
                    ...
                    # Display sound wave visualization
                    st.markdown("### Sound Wave Visualization:")
                    audio_data, sr = librosa.load(audio_path)  # Load the WAV file
